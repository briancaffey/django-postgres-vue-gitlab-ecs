<template>
  <base-page>
    <h4>Redis Test</h4>
    <p class="redis-debug">Value: <span id="val">{{ valueFromCache }}</span></p>
      <q-input
        id="input"
...
<script>
  ...
  export default {
    ...
    methods: {
      clearCacheValue() {
        apiCall.delete("/api/debug/redis").then(
          (resp) => {
            console.log(resp);
      ...
      getCachedValue() {
        apiCall.get("/api/debug/redis/").then(
          (resp) => {
